...
export class ServicesService {
  ...
  public getCurrentUser() {
    return this.http.get('http://localhost:4000/current-user');
  }

  //register User
  public registerUser(user: User): Observable<any> {
    return this.http.post<any>('http://localhost:4000/user/register', user);
  }
  //Search product by plain text
  public searchProducts(query: String) {
    return this.http.get('http://localhost:4000/products?query=' + query);
  }

  // show Reviews for a product
  public showReviews(productId: any) {
    return this.http.get(
      'http://localhost:4000/products/' + productId + '/showReviews'
    );
  }

  // Add new review
  public addReview(reviewData: any) {
    return this.http.post('http://localhost:4000/addReview', reviewData);
  }
  ...
  public showStats(): Observable<any> {
    return this.http.get<any>('http://localhost:4000/home/stats');
  }
  ...
  public showProducts() {
    return this.http.get('http://localhost:4000/user/products');
  }

  //show all reviews in user
  public showAllReviews() {
    return this.http.get('http://localhost:4000/user/reviews');
  }

  //show all reviews in admin which are not approved yet
  public showAdminReviews() {
    return this.http.get('http://localhost:4000/admin/reviews');
  }

  // update the review and approve it
  public approveReview(review: any) {
    return this.http.put('http://localhost:4000/review/approve', review);
  }

  //update the user status active or non-active
  public userSatus(user: any) {
    return this.http.put('http://localhost:4000/user/active', user);
  }

  //to request a review
  public reviewRequest(product: any) {
    return this.http.post('http://localhost:4000/review/request', product);
  }

  public cancelReview(review:any){
    return this.http.put('http://localhost:4000/review/cancel',review);
  }
  
//   private baseUrl = 'http://localhost:4000/api/products';

// public  searchProduct(query: string): Observable<Product[]> {
